...
function Lyricsedit({
  ...
  const handleTimeChange = useCallback(
    (time, index) => {
      setChanged(true)
      // updateTimeDispath(time, index)
    },
    [setChanged, updateTimeDispath]
  )
  ...
  const handlerUploadBtn = useCallback(() => {
    uploadBoxShowDispath(true)
    playPauseDispath(true)
  }, [uploadBoxShowDispath, playPauseDispath])
  const handlerEditBtn = useCallback(() => {
    setEditedDispath(true)
    updateCurrentIndexDispath(-1)
  }, [setEditedDispath, updateCurrentIndexDispath])
  const handlerOverviewBtn = useCallback(() => {
    setEditedDispath(false)
    updateCurrentIndexDispath(-1)
  }, [setEditedDispath, updateCurrentIndexDispath])
  const handlerCopyBtn = useCallback(() => {
  ...
  }, [lyricsState, lytimesState, hinted.clearTimeId])
              ...
              {lyricsState.map((ly, index) => (
                <li
                  ...
                  onMouseLeave={() => {
                    ...
                    if (clickIndex === index && checked && changed) {
                      ...
                      setChecked(false)
                      // 将数据同步到状态上
                      // updateTimeDispath(lytimesState[index], index)
                    }
                    if (clickIndex !== -1) {
                      setClickIndex(-1)
                    }
                ...
                >
                  <div
                    ...
                    style={{
                      width: editedState || clickIndex === index ? '24%' : '0%',
                    }}
                  >
                    [
                    <input
                      type="text"
                      value={lytimesState[index]}
                      onChange={(evt) => {
